...
public final class ConfidenceValidator {
     ...
     * @param scores Map of value key to total score
    ...
    public ClassificationResult validate(
        ...
        if (sorted.isEmpty()) {
            return ClassificationResult.empty(itemId, "default");
        }
    ...
    public static ConfidenceValidator withDefaultThreshold() {
        return new ConfidenceValidator(
            MarieLibContext.isRegistered() ? MarieLibContext.get().scannerConfidenceSpreadThreshold() : 0f
        );
    }
}
